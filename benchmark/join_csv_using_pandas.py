...
for f in other_csvs:
    if path.exists(f):
        ...
        is_failed = (
            (df_current["obj"] == float("-inf"))
            | (df_current["obj"] == "NAN")
            | (df_current["obj"] == "FAILED")
        )
        ...
        df_merged = pd.merge(
            df_merged, df_current, how="left", left_on="name", right_index=True
        )
    else:
...
obj_cols = [
    col for col in obj_cols if "sbmiqp" in col or "sbmiqp_ee" in col or "bonmin" in col
]
