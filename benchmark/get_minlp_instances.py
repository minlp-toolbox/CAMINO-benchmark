...
instance_df = instance_df.loc[
    (instance_df["probtype"] == "MINLP") | (instance_df["probtype"] == "MBNLP")
]

mask = ((instance_df["objsense"] == "min") | (instance_df["objsense"] == "max")) & (
    instance_df["formats"].str.contains("'nl'")
)
...
print(
    f"csv with convex instances saved at {os.path.join(dirpath, 'convex_set_full.csv')}"
)
print(
    f"csv with nonconvex instances saved at {os.path.join(dirpath, 'nonconvex_set_full.csv')}"
)
